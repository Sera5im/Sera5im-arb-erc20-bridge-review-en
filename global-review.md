...
- destination-side asset credit or release must not happen without source-side accounting
...
- disabled hook and extra-data branches must not silently change normal bridge semantics
...
- recovery and fallback branches must not conflict with the normal branch
- the privileged, config, init, and upgrade surface must not silently break bridge guarantees
